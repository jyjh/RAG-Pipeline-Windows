...
def create_app(config: PipelineConfig | None = None):
    ...
    def require_token(x_api_token: str | None = Header(default=None)) -> None:
        if cfg.server.lan and cfg.server.api_token:
            if x_api_token != cfg.server.api_token:
    ...
    def health() -> dict[str, Any]:
        return {
            "status": "ok",
            "db": str(Path(cfg.paths.db_dir) / "rag.sqlite"),
    ...
    def document_blocks(doc_id: str, _: None = Depends(require_token)) -> list[dict[str, Any]]:
    ...
    def ingest(request: IngestRequest, _: None = Depends(require_token)) -> dict[str, Any]:
        source = request.path or cfg.paths.data_dir
        ingestor = StructuredIngestor(cfg, store)
        doc_ids = ingestor.ingest_path(source)
    ...
    def query(request: QueryRequest, _: None = Depends(require_token)) -> dict[str, Any]:
        response = query_service.ask(
            ...
            mode=request.mode,
            top_k=request.top_k,
            doc_filters=request.document_ids,
        )
